...
public class MediationController {
	...
	public @ResponseBody
	boolean addCommento(HttpServletRequest request,@RequestBody String messageToMediationService) {
		MessageToMediationService mediationService=MessageToMediationService.valueOf(messageToMediationService);
		
		db.save(mediationService);		
		
		
		Query query2 = new Query();
		query2.addCriteria(Criteria.where("entityId").is(mediationService.getEntityId()));
	...
	public @ResponseBody List<MessageToMediationService> getCommento(HttpServletRequest request) {
	...
	public @ResponseBody
	boolean addNote(HttpServletRequest request,@PathVariable String _id,@RequestParam String note) {
		
		
		Query query2 = new Query();
		query2.addCriteria(Criteria.where("_id").is(_id));
		MessageToMediationService mediationService=db.findOne(query2, MessageToMediationService.class);
	...
	public @ResponseBody
	boolean parseApprovedChange(HttpServletRequest request,@PathVariable String _id) {
	...
	public @ResponseBody
	boolean mediationApprovedChange(HttpServletRequest request,@PathVariable String _id) {
		...
		mediationService.setMediationApproved(!mediationService.isMediationApproved());
